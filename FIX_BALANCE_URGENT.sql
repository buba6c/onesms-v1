-- =====================================================
-- FIX URGENT: Remettre le solde et frozen corrects
-- Puis corriger le système atomic_refund
-- =====================================================

-- ÉTAPE 1: Remettre le solde correct pour buba6c
UPDATE users 
SET 
  balance = 21,
  frozen_balance = 7,
  updated_at = NOW()
WHERE id = 'e108c02a-2012-4043-bbc2-fb09bb11f824';

-- Vérifier
SELECT email, balance, frozen_balance 
FROM users 
WHERE id = 'e108c02a-2012-4043-bbc2-fb09bb11f824';

-- ÉTAPE 2: S'assurer que frozen_amount est correct sur les activations pending
UPDATE activations
SET frozen_amount = price
WHERE user_id = 'e108c02a-2012-4043-bbc2-fb09bb11f824'
  AND status = 'pending'
  AND (frozen_amount IS NULL OR frozen_amount = 0);

-- Vérifier les activations
SELECT id, status, price, frozen_amount, created_at
FROM activations
WHERE user_id = 'e108c02a-2012-4043-bbc2-fb09bb11f824'
ORDER BY created_at DESC
LIMIT 10;

-- =====================================================
-- ÉTAPE 3: Corriger atomic_refund pour être plus sécurisé
-- Le problème: si frozen_amount=0, il faut utiliser price
-- =====================================================
CREATE OR REPLACE FUNCTION atomic_refund(
  p_user_id UUID,
  p_activation_id UUID DEFAULT NULL,
  p_rental_id UUID DEFAULT NULL,
  p_transaction_id UUID DEFAULT NULL,
  p_reason TEXT DEFAULT NULL
)
RETURNS JSON AS $$
DECLARE
  v_user RECORD;
  v_frozen_amount DECIMAL;
  v_price DECIMAL;
  v_refund DECIMAL;
  v_new_balance DECIMAL;
  v_new_frozen DECIMAL;
BEGIN
  -- 1. LOCK USER
  SELECT balance, frozen_balance
  INTO v_user
  FROM users
  WHERE id = p_user_id
  FOR UPDATE;
  
  IF NOT FOUND THEN
    RAISE EXCEPTION 'User not found: %', p_user_id;
  END IF;
  
  -- 2. GET frozen_amount AND price FROM ACTIVATION OR RENTAL
  IF p_activation_id IS NOT NULL THEN
    SELECT COALESCE(frozen_amount, 0), COALESCE(price, 0) 
    INTO v_frozen_amount, v_price
    FROM activations
    WHERE id = p_activation_id
    FOR UPDATE;
    
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Activation not found: %', p_activation_id;
    END IF;
    
    -- ✅ FIX: Si frozen_amount=0, utiliser price (activation déjà partiellement refunded)
    IF v_frozen_amount = 0 AND v_price > 0 THEN
      -- Ne rien rembourser car déjà fait
      RETURN json_build_object(
        'success', true,
        'message', 'Already refunded (frozen_amount=0)',
        'refunded', 0,
        'balance', v_user.balance,
        'frozen', v_user.frozen_balance
      );
    END IF;
    
  ELSIF p_rental_id IS NOT NULL THEN
    SELECT COALESCE(frozen_amount, 0), COALESCE(total_cost, 0) 
    INTO v_frozen_amount, v_price
    FROM rentals
    WHERE id = p_rental_id
    FOR UPDATE;
    
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Rental not found: %', p_rental_id;
    END IF;
    
    IF v_frozen_amount = 0 AND v_price > 0 THEN
      RETURN json_build_object(
        'success', true,
        'message', 'Already refunded (frozen_amount=0)',
        'refunded', 0,
        'balance', v_user.balance,
        'frozen', v_user.frozen_balance
      );
    END IF;
  ELSE
    RAISE EXCEPTION 'Either activation_id or rental_id must be provided';
  END IF;
  
  -- 3. CALCULATE REFUND - UNIQUEMENT le frozen_amount de cette activation
  -- Ne pas utiliser price car ça pourrait rembourser plus que prévu
  v_refund := LEAST(v_frozen_amount, v_user.frozen_balance);
  
  IF v_refund <= 0 THEN
    RETURN json_build_object(
      'success', true,
      'message', 'Nothing to refund',
      'refunded', 0,
      'balance', v_user.balance,
      'frozen', v_user.frozen_balance
    );
  END IF;
  
  v_new_balance := v_user.balance + v_refund;
  v_new_frozen := GREATEST(0, v_user.frozen_balance - v_refund);
  
  -- 4. UPDATE USER (UNFREEZE + REFUND)
  UPDATE users
  SET 
    balance = v_new_balance,
    frozen_balance = v_new_frozen,
    updated_at = NOW()
  WHERE id = p_user_id;
  
  -- 5. RESET frozen_amount AND UPDATE STATUS
  IF p_activation_id IS NOT NULL THEN
    UPDATE activations
    SET 
      frozen_amount = 0,
      status = CASE 
        WHEN status IN ('pending', 'waiting', 'active') THEN 'cancelled'
        ELSE status 
      END,
      updated_at = NOW()
    WHERE id = p_activation_id;
  END IF;
  
  IF p_rental_id IS NOT NULL THEN
    UPDATE rentals
    SET 
      frozen_amount = 0,
      status = CASE 
        WHEN status = 'active' THEN 'cancelled'
        ELSE status 
      END,
      updated_at = NOW()
    WHERE id = p_rental_id;
  END IF;
  
  -- 6. UPDATE TRANSACTION
  IF p_transaction_id IS NOT NULL THEN
    UPDATE transactions
    SET status = 'refunded', updated_at = NOW()
    WHERE id = p_transaction_id AND status = 'pending';
  END IF;
  
  -- 7. LOG OPERATION
  INSERT INTO balance_operations (
    user_id, 
    activation_id,
    rental_id,
    related_transaction_id,
    operation_type, 
    amount,
    balance_before, 
    balance_after,
    frozen_before,
    frozen_after,
    reason
  ) VALUES (
    p_user_id,
    p_activation_id,
    p_rental_id,
    p_transaction_id,
    'refund',
    v_refund,
    v_user.balance,
    v_new_balance,
    v_user.frozen_balance,
    v_new_frozen,
    COALESCE(p_reason, 'Activation/Rental refund')
  );
  
  RETURN json_build_object(
    'success', true,
    'refunded', v_refund,
    'balance', v_new_balance,
    'frozen', v_new_frozen
  );
EXCEPTION
  WHEN OTHERS THEN
    RETURN json_build_object(
      'success', false,
      'error', SQLERRM
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
