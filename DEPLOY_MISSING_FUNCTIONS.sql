-- ============================================================================
-- DÉPLOIEMENT DES FONCTIONS SQL MANQUANTES
-- Date: 2025-12-03
...
-- Gèle des fonds pour une activation/rental
...
BEGIN
  ...
  -- Mettre à jour frozen_amount sur activation/rental
  ...
  RETURN json_build_object(
    'success', true,
    'frozen', p_amount,
    'new_frozen_balance', v_user.frozen_balance + p_amount
  );
END;
$$;
...
-- Dégèle des fonds (refund ou commit)
...
BEGIN
  ...
  -- Décrémenter frozen_balance
  UPDATE users
  SET frozen_balance = GREATEST(0, frozen_balance - v_frozen_amount),
      updated_at = NOW()
  WHERE id = p_user_id;
  ...
  -- Mettre frozen_amount à 0
  IF p_activation_id IS NOT NULL THEN
    UPDATE activations 
    SET frozen_amount = 0, updated_at = NOW() 
  ...
  RETURN json_build_object(
    ...
    'new_frozen_balance', GREATEST(0, v_user.frozen_balance - v_frozen_amount)
  );
END;
$$;
...
-- Remboursement atomique avec idempotence
...
DECLARE
  v_result JSON;
BEGIN
  -- Appeler secure_unfreeze_balance avec refund_to_balance=true
  v_result := secure_unfreeze_balance(
    p_user_id,
    p_activation_id,
    p_rental_id,
    TRUE,  -- refund_to_balance
    p_reason
  );

  -- Si succès, mettre à jour le status
  ...
  RETURN json_build_object(
    ...
    'refunded', (v_result->>'unfrozen')::decimal,
...
-- Commit atomique (débiter définitivement après SMS reçu)
...
BEGIN
  ...
  -- Décrémenter frozen_balance (Model A: ne pas débiter balance)
  UPDATE users
  SET frozen_balance = GREATEST(0, frozen_balance - v_frozen_amount),
      updated_at = NOW()
  WHERE id = p_user_id;
  ...
  INSERT INTO balance_operations (
    ...
    p_user_id, p_activation_id, p_rental_id, 'charge', v_frozen_amount,
    v_user.balance, v_user.balance,
    v_user.frozen_balance, GREATEST(0, v_user.frozen_balance - v_frozen_amount),
    p_reason
  );

  -- Mettre frozen_amount à 0 et charged=true
  IF p_activation_id IS NOT NULL THEN
    UPDATE activations
...
-- ============================================================================
-- PARTIE 2: PROCESS EXPIRED ACTIVATIONS
-- ============================================================================
...
BEGIN
  ...
  FOR v_activation IN
    SELECT 
      id, user_id, price, frozen_amount, order_id, service_code, expires_at
    FROM activations 
...
-- ============================================================================
-- PARTIE 3: VUES DE MONITORING
-- ============================================================================

-- Vue: Santé des balances gelées
CREATE OR REPLACE VIEW v_frozen_balance_health AS
SELECT
  (SELECT COALESCE(SUM(frozen_amount), 0) FROM activations WHERE status IN ('pending', 'waiting') AND frozen_amount > 0) +
  (SELECT COALESCE(SUM(frozen_amount), 0) FROM rentals WHERE status = 'active' AND frozen_amount > 0) 
  ...
  (SELECT COUNT(*) FROM activations WHERE status IN ('pending', 'waiting') AND frozen_amount > 0) AS active_activations,
  (SELECT COUNT(*) FROM rentals WHERE status = 'active' AND frozen_amount > 0) AS active_rentals;
