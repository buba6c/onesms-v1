...
async function testRentStatus() {
  ...
  // 1. Get user using ilike
  const { data: users, error } = await supabase
    .from('users')
    .select('id, email')
    .ilike('email', '%buba6c%');
  
  console.log('Users found:', users, error);
  
  if (!users || users.length === 0) {
    console.log('❌ User not found');
    // Get rentals directly
    const { data: allRentals } = await supabase
      .from('rentals')
      .select('id, user_id, phone, status, order_id, rent_id, rental_id')
      .order('created_at', { ascending: false })
      .limit(5);
    
    console.log('Recent rentals:', allRentals);
    return;
  }
