...
```typescript
const { data: dbRentals = [], refetch: refetchRentals } = useQuery<
  ActiveNumber[]
>({
...
const countResponse = await fetch(
  `${SUPABASE_URL}/rest/v1/rentals?select=count`,
  {
...
const rentalsResponse = await fetch(
  `${SUPABASE_URL}/rest/v1/rentals?order=created_at.desc&limit=3&select=*`,
  {
    headers: { apikey: ANON_KEY, Authorization: `Bearer ${ANON_KEY}` },
  }
);
