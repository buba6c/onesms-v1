...
serve(async (req) => {
  ...
  try {
    ...
    try {
      const v2Url = `https://hero-sms.com/stubs/handler_api.php?api_key=${SMS_ACTIVATE_API_KEY}&action=getStatus&id=${orderId}`
    ...
    try {
      const historyUrl = `https://hero-sms.com/stubs/handler_api.php?api_key=${SMS_ACTIVATE_API_KEY}&action=getFullSms&id=${orderId}`
